...
class ExploreHelper{
  ...
  Future<dynamic> requestExploreSection(BookExploreUrlBean options,OnBookListLoad? onBookListLoad,UpdateList? updateList,BookSourceBean sourceBean) async{
    ...
    try{
      ...
      if(e is DioError){
        ...
        if(rsp?.statusCode == 302){
          ...
          options.method = "GET";
          options.body = "";
          return requestExploreSection(options, onBookListLoad,updateList, sourceBean);
        }
      }
  ...
  dynamic _parseResponse(String response,BookSourceBean sourceBean, OnBookListLoad? onBookListLoad) async{
    ...
    try{
      ...
      print('解析返回内容开始：$sourceId|${DateTime.now().difference(tempTime).inMilliseconds}');
      //用线程池执行解析，大概需要400ms
      var tmp = await Executor().execute(arg1:kv,fun1: _parse);
      print('解析返回内容结束：$sourceId|${DateTime.now().difference(tempTime).inMilliseconds}');
