            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  ListTile(title: Text("数据库"),subtitle: Text("点击清空书籍缓存"),trailing: Icon(Icons.delete_forever_rounded),onTap: () async{
                    var result = await _showClearDatabaseDialog(context);
                    if(result == "done"){
                      await DatabaseHelper().clearBookData();
                  ...
                  AboutListTile(applicationName: '扑哒小说',applicationVersion: 'ver 2.4.0',applicationLegalese: '开源地址\nhttps://github.com/huage2580/yuedu_hd\n参考实现[阅读安卓版]:\n https://github.com/gedoor/legado',),
...
When using the PUDA Read network service to conduct online transactions, you will inevitably disclose your personal information, such as contact information or postal address, to the counterparty or potential counterparty. Please properly protect your personal information and only provide it to others when necessary. If you find that your personal information has been leaked, especially when PUDA Read username and password are leaked, please contact the PUDA Read customer service immediately so that PUDA Read can take corresponding measures.
                  ...
                  ListTile(title: Text('免责声明'),trailing: Icon(Icons.arrow_forward_ios_rounded),onTap: (){
                    pushTextPage('免责声明', '''
    扑哒小说(以下简称为扑哒)是一款提供网络文学搜索的工具，为广大网络文学爱好者提供一种方便、快捷舒适的试读体验。

    当您搜索一本书的时，扑哒会将该书的书名以关键词的形式提交到第三方网络文学网站。第三方网站返回的内容与扑哒无关，扑哒对其概不负责，亦不承担任何法律责任。任何通过使用扑哒而链接到的第三方网页均系他人制作或提供，您可能从第三方网页上获得其他服务，扑哒对其合法性概不负责，亦不承担任何法律责任。第三方搜索引擎结果根据您提交的书名自动搜索获得并提供试读，不代表扑哒赞成或被搜索链接到的第三方网页上的内容或立场。您应该对使用搜索引擎的结果自行承担风险。
  ...
  dynamic _showClearDatabaseDialog(context) async{
    return await showDialog(context: context, builder: (context){
      return AlertDialog(title: Text("警告"),content: Text("确定清除所有书籍？"),actions: [
